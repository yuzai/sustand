...
/** 筛选 suspense 的 keys */
export type FilterSuspenseKey<T> =
    Filter<T, { action: (...args: any) => any, sustand_internal_issuspense: boolean }>;

/** 筛选 computed 的 keys */
export type FilterComputedKey<T> =
    Filter<T, { action: (...args: any) => any, sustand_internal_iscomputed: boolean }>;
...
export type UseStore<T> = {
    <F extends (state: Convert<T>) => any>
    (selector: F, equalityFn?: EqualityFn<ReturnType<F>>): ReturnType<F>;
...
/** 为原始 zustand 准备的类型声明 */
export type StateCreatorMiddware<T extends {}> =
