...
/** 筛选 suspense 的 keys */
export type FilterSuspenseKey<T> = Filter<T, {
    action: (...args: any) => any;
    sustand_internal_issuspense: boolean;
}>;
/** 筛选 computed 的 keys */
export type FilterComputedKey<T> = Filter<T, {
    action: (...args: any) => any;
    sustand_internal_iscomputed: boolean;
}>;
...
export type UseStoreSuspense<T, O = {
    ...
    loadScript: undefined | React.DetailedReactHTMLElement<{
...
export type UseStore<T> = {
    ...
    <S extends SuspensedConvert<T>, K extends FilterSuspenseKey<T>>(key: K, options?: any): {
        data: S[K]['data'];
        status: Status;
        error: any;
        refresh: (force?: boolean) => Promise<S[K]['data']>;
...
export type SetState<T> = {
    (partial: T | Partial<T> | ((state: T) => T | Partial<T>), desc?: string, replace?: boolean): void;
};
